...
const AdminIdxAttendance = (props: { employees: EmployeeModel[] }) => {
  const { user } = useAuth();
        ...
        <Table>
          ...
          <TableHead>
            <TableRow>
              ...
              <TableCell style={{ width: "16%" }} align="center">
                성명
              </TableCell>
              <TableCell style={{ width: "11%", color: "red" }} align="center">
                일
              </TableCell>
              ...
              <TableCell style={{ width: "11%", color: "blue" }} align="center">
                토
              </TableCell>
            </TableRow>
          </TableHead>
              ...
              .map((data, idx) => (
                <TableRow key={data._id}>
                  ...
                  <TableCell align="center">
                    {data.name}({(data.department as DepartmentModel).name})
                  </TableCell>
                  {[0, 1, 2, 3, 4, 5, 6].map(val => {
                    const date = addDays(baseDate, val);
                    ...
                    return (
                      <TableCell align="center" key={date.getTime()}>
                        <Select
                          value={value}
                          disabled={moment().diff(date, "days", true) < 0 || moment(data.join).diff(date, "days") > 0}
...
AdminIdxAttendance.getInitialProps = async (ctx: NextPageContext) => {
...
export default withLayout(AdminIdxAttendance, { title: "출근부" });
